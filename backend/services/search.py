...
class LangChainSearchService:
    """Production-grade search using DDGS + AsyncHtmlLoader."""
    ...
    def __init__(self):
        ...
        # Initialize search with list output
    ...
    async def search_and_fetch(self, query: str, max_results: int = 5) -> List[Document]:
        """Complete async search and fetch using components."""
        try:
            start_time = time.time()
            logger.info(f"DDGS search: {query[:50]}...")
            
            # Step 1: Async DDGS search
            ...
            # Step 3: AsyncHtmlLoader for content extraction
            if urls:
                ...
                if documents:
                    ...
                    logger.info(f"✅ pipeline: {len(documents)}/{len(urls)} docs ({success_rate:.1f}%) in {search_time:.2f}s")
                else:
    ...
    async def _extract_content_with_langchain(
        ...
        """Extract content"""
        try:
            logger.info(f"Content extraction: {len(urls)} URLs")

            # Use AsyncHtmlLoader but with better cleaning
